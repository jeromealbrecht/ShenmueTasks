...
import { CheckSquare, Square, PlusCircle, Trash2, Edit3, Save, XCircle } from "lucide-react"
...
export default function ShenmueTaskCarnet() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error("Erreur lors du chargement des tâches depuis le localStorage:", error)
      setTasks(DEFAULT_TASKS)
    } finally {
      setIsLoading(false)
    }
  ...
  useEffect(() => {
    if (!isLoading && isClient) {
      try {
        ...
        console.error("Erreur lors de la sauvegarde des tâches dans le localStorage:", error)
      }
    }
        ...
        if (playPromise !== undefined) {
          playPromise.catch((e) => {
            ...
            if (e.name === "NotSupportedError") {
              console.error("Vérifiez que le fichier /audio/shen.mp3 est accessible et au bon format.")
            }
          })
        }
      }, 300)
  ...
  const handleAddTask = () => {
    ...
    setTasks((prevTasks) => [...prevTasks, { id: Date.now(), text: newTask.trim(), done: false }])
    setNewTask("")
  }
  ...
  const handleSaveEdit = () => {
    ...
    setTasks((prevTasks) =>
      prevTasks.map((task) => (task.id === editingTaskId ? { ...task, text: editingTaskText.trim() } : task)),
    )
    handleCancelEdit()
  }
  ...
  return (
    <div className="relative perspective-[1000px] max-w-xl mx-auto my-10 p-4 sm:p-0">
      <motion.div
        ...
        className="bg-paper bg-[url('https://www.transparenttextures.com/patterns/paper-fibers.png')] p-6 sm:p-8 rounded-xl shadow-2xl border-2 border-neutral-700 relative min-h-[300px] flex flex-col"
        style={{ backgroundBlendMode: "multiply", backgroundColor: "var(--color-paper)" }}
      >
        {isClient && (
        ...
        <h1 className="text-3xl sm:text-4xl font-hand text-ink mb-6 flex items-center gap-2">
          <motion.span
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
        ...
        {tasks.length === 0 && !isLoading && isClient ? (
          ...
          >
            ...
            <p className="text-lg opacity-70">Il est temps d'enquêter ou d'ajouter une nouvelle mission.</p>
          </motion.div>
        ) : (
              ...
              {tasks.map((task) => (
                ...
                >
                  ...
                  {editingTaskId === task.id ? (
                    ...
                    <span
                      className={`flex-1 text-lg font-hand text-ink cursor-pointer ${task.done ? "line-through opacity-60" : ""}`}
      ...
      {isClient && <audio ref={audioRef} src="/audio/shen.mp3" preload="auto" />}
    </div>
  )
}
